...
int subsetSumMultipleQueries(int level, int sumLeft) {
    ...
    // Transition 
    int answer = 0;
    if (subsetSumMultipleQueries(level + 1, sumLeft) || subsetSumMultipleQueries(level + 1, sumLeft - input[level]))
        answer = 1;
...
void printSubset(int level, int sumLeft) {
    if (level == n + 1) 
        return;
    if (subsetSumMultipleQueries(level + 1, sumLeft))
...
int main() {
    ...
    while(queries-- > 0) {
        cin >> constraintSum;
        if (subsetSumMultipleQueries(1, constraintSum)) 
            printSubset(1, constraintSum);
        else
            cout << "No Solution\n";
    }
