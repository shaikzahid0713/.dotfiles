...
int longestIncreasingSubsequenceFirstForm(int level, int lastIndex) {
    ...
    // Transition
    int ans = longestIncreasingSubsequenceFirstForm(level + 1, lastIndex);
    if (lis[level] > lis[lastIndex])
        answer = max(ans, lis[level] + longestIncreasingSubsequenceFirstForm(level + 1, level));
...
int main() {
    ...
    int firstForm = longestIncreasingSubsequenceFirstForm(1, 0);
    cout << firstForm << "\n";
