...
int longestIncreasingSubsequenceFirstForm(int level, int lastIndex) {
    ...
    // Save and Return
    return dpf[level] = answer;
}

int longestIncreasingSubsequenceFirstFormRecursive(int level, int lastIndex) {
    // Base Case
    if (level == n) 
        return 0;
    
    // Compute 
    int answer = longestIncreasingSubsequenceFirstFormRecursive(level + 1, lastIndex);
    
    if (lis[level] > lis[lastIndex])
        answer = max(answer, lis[level] + longestIncreasingSubsequenceFirstFormRecursive(level + 1, level));
...
int longestIncreasingSubsequence(int level) {
    ...
    for (int previousTaken = 0; previousTaken < level; previousTaken++) {
        if (lis[level] > lis[previousTaken]) 
            answer = max(answer, lis[level] + longestIncreasingSubsequence(previousTaken));
    }
...
int main() {
    ...
    int firstFormRecursive = longestIncreasingSubsequenceFirstFormRecursive(0, -1);
    cout << firstFormRecursive << "\n";
    
    int firstForm = longestIncreasingSubsequenceFirstForm(0, -1);
    cout << firstForm << "\n";
